...
async function getTodayAttendance() {
  ...
  const { data, error } = await supabaseAdmin
    .from("attendance")
    .select(
      "*, profiles(id, full_name, email, nisn, school, major), locations(name)",
    )
...
const getAttendanceStats = async () => {
  ...
  // Ambil semua siswa dengan role user
  ...
  const present = checkedIn + completed;

  const notPresent = Math.max(totalStudents - present, 0);
