...
/**
 * Menghitung durasi presensi
 * @param {string} checkInTime
 * @param {string|null} checkOutTime
 * @returns {{duration_minutes: number|null, duration_text: string|null}}
 */
...
async function getTodayAttendance(userId) {
  ...
  const duration = calculateDuration(data?.check_in_time, data?.check_out_time);
  const activeLocation = await locationService.getActiveOfficeLocation();
...
async function checkIn(userId, payload) {
  ...
  if (
    ...
    throw {
      ...
      message:
        "Data koordinat (latitude, longitude, dan accuracy) wajib disertakan.",
    };
  }
  ...
  const validation = await locationService.validateGpsAndRadius(
    latitude,
    longitude,
    accuracy,
  );
  ...
  let finalPhotoPath =
    photo_path || "attendance-photos/default-placeholder.jpg";
  const photoInput = payload.photo || payload.photo_base64;
  if (photoInput) {
    const uploadResult = await storageService.uploadAttendancePhoto(
      userId,
      photoInput,
      "check-in",
    );
...
async function getAttendanceById(attendanceId, userId, isAdmin = false) {
  ...
  let query = supabaseAdmin
    .from("attendance")
    .select(
      "*, profiles(full_name, email, nisn, school, major), locations(name)",
    )
    .eq("id", attendanceId);
  ...
  if (error || !data) {
    throw {
      ...
      message:
        "Data presensi tidak ditemukan atau Anda tidak memiliki izin untuk melihatnya.",
    };
  }
