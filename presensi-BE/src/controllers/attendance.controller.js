 ...
 * Mengambil koordinat dan radius aktif kantor dari database
 */
async function getLocation(req, res, next) {
  try {
    const location = await locationService.getActiveOfficeLocation();
    return successResponse(res, "Data lokasi aktif kantor berhasil dimuat.", { location }, 200);
  } catch (error) {
    if (error.code && error.statusCode) {
      return errorResponse(res, error.message, error.code, error.statusCode, error.details);
    }
