...
async function runTests() {
  ...
  try {
    ...
    // Anti-cheat: Akurasi terlalu lemah (> 100m)
    try {
      await locationService.validateGpsAndRadius(officeLat, officeLng, 125);
      assert(false, "Akurasi 125 meter harus ditolak");
    } catch (err) {
      assert(err.code === "GPS_ACCURACY_TOO_LOW", "Akurasi sinyal lemah berhasil ditolak (GPS_ACCURACY_TOO_LOW saat accuracy > 100m)");
    }
    ...
    // -------------------------------------------------------------
    console.log("\n[6] Pengujian Telegram Service & Real-Time Logger");
    // -------------------------------------------------------------
    const telegramService = require("../src/services/telegram.service");
    
    // Test WIB Formatting
    const testDate = new Date("2026-09-07T00:15:00.000Z"); // 07:15 WIB
    const { dateFormatted, timeFormatted } = telegramService.formatDateTimeWIB(testDate);
    assert(timeFormatted.includes("WIB"), `Waktu presensi terformat WIB: ${timeFormatted}`);
    assert(dateFormatted.includes("2026"), `Tanggal presensi terformat tahun 2026: ${dateFormatted}`);

    // Test HTML Escaping
    const escaped = telegramService.escapeHtml("<script>alert('test')&</script>");
    assert(escaped === "&lt;script&gt;alert('test')&amp;&lt;/script&gt;", "Sanitasi HTML escape berjalan dengan aman");

    // Test sendAttendanceLog non-blocking saat unconfigured
    const logResult = await telegramService.sendAttendanceLog({
      student: { full_name: "Dennis Ali Fadillah", nisn: "242510046", school: "SMK Taruna Bhakti", major: "Rekayasa Perangkat Lunak" },
      type: "CHECK_IN",
      time: testDate.toISOString(),
      distance: 2.3,
      accuracy: 20.1,
      locationName: "Direktorat Bina Teknik Sumber Daya Air",
      photoUrl: "https://cvwrechqivfufycjmbsb.supabase.co/storage/v1/object/public/attendance-photos/test.jpg",
    });
    assert(logResult === null || typeof logResult === "object", "sendAttendanceLog berjalan non-blocking dan aman saat unconfigured");

    // Test health telegram endpoint
    const teleHealthRes = await (await fetch(`${BASE_URL}/api/health/telegram`)).json();
    assert(teleHealthRes.hasOwnProperty("success"), "GET /api/health/telegram merespons format JSON standar");
