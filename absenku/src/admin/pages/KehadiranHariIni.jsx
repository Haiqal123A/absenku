...
export default function KehadiranHariIni() {
   ...
   * Ini digunakan supaya ketika status siswa Izin/Sakit,
   * otomatis masuk ke halaman Pengajuan.
   */
  ...
  const normalizedStudents = useMemo(() => {
    ...
    return students.map((student, index) => {
      ...
      const lokasiKerja = wfhToday
        ? "wfh"
        : normalizeLocation(lokasiAwal);

      /* STATUS berdasarkan absensi + jadwal admin */
      ...
      /* JAM ABSENSI ASLI */
      const attendance = getDisplayAttendance(
        student,
        settings,
        now,
      );
  ...
  /* =========================================================
     OTOMATIS BUAT PENGAJUAN IZIN / SAKIT
     
     Jika status absensi hari ini:
     - Izin
     - Sakit

     maka otomatis dibuat data pengajuan.

     Tidak akan membuat duplikat untuk:
     siswa + tanggal + jenis.
  ========================================================= */

  useEffect(() => {
    if (
      !Array.isArray(normalizedStudents) ||
    ...
    normalizedStudents.forEach((student) => {
      ...
      const studentNis = String(
        student.nis ?? "-"
      );

      /*
       * Cek apakah pengajuan dengan siswa,
       * tanggal, dan jenis yang sama sudah ada.
       */
      ...
      /*
       * Cek juga terhadap data baru yang sedang
       * dibuat pada proses ini supaya tidak duplikat.
       */
      ...
      newRequests.push({
        id: `absensi-${student.id}-${today}-${student.status.toLowerCase()}`,
        ...
        alasan:
          ...
          student.keterangan ||
          student.reason ||
          `Absensi otomatis tercatat sebagai ${student.status}.`,
  ...
  const totalWFH =
    normalizedStudents.filter(
      (student) =>
        student.lokasiKerja === "wfh",
    ).length;

  const totalKantor =
    normalizedStudents.filter(
      (student) =>
        student.lokasiKerja === "kantor",
    ).length;
            ...
            <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Jam masuk dan pulang mengikuti data absensi siswa.
                  Status terlambat mengikuti jadwal Admin.
                </p>
              </div>
